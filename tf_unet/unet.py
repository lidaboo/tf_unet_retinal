...
def create_conv_net(x, keep_prob, channels, n_class, layers=5, features_root=16, filter_size1=7, filter_size2=3, pool_size=2, summaries=True):
    """
    ...
    logging.info("Layers {layers}, features {features}, filter size {filter_size1}x{filter_size2}, pool size: {pool_size}x{pool_size}".format(layers=layers,
    ...
    for layer in range(0, layers):
        ...
        stddev = np.sqrt(2 / (filter_size1**2 * features))
        if layer == 0:
            w1 = weight_variable([filter_size1, filter_size2, channels, features], stddev)
        else:
            w1 = weight_variable([filter_size1, filter_size2, features//2, features], stddev)
            
        w2 = weight_variable([filter_size1, filter_size2, features, features], stddev)
    ...
    for layer in range(layers-2, -1, -1):
        ...
        w1 = weight_variable([filter_size1, filter_size2, features, features//2], stddev)
        w2 = weight_variable([filter_size1, filter_size2, features//2, features//2], stddev)
